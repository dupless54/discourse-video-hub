...
module("Integration | Component | VideoHubCollections add video", function (hooks) {
  ...
  test("loads the catalog lazily, paginates without duplicates, and adds a canonical video", async function (assert) {
    ...
    pretender.get("/videos/collections/42/catalog.json", (request) => {
      ...
      if (request.queryParams.cursor === "90") {
        return response(200, {
          ...
          videos: [catalogVideo(91, "Already shown"), catalogVideo(90, "Second page")],
    ...
    await render(
      <template><VideoHubCollectionsManager @model={{collectionModel}} /></template>
    );
  ...
  test("shows the owner-only series catalog explanation", async function (assert) {
    const model = collectionModel({ collection_type: "series", title: "Episodes" });

...
module("Integration | Component | VideoHubLanding | immersive explore", function (hooks) {
  ...
  test("keeps one active player, prefetches near the end, dedupes, and supports keyboard navigation", async function (assert) {
    ...
    pretender.get("/videos/feed.json", (request) => {
      ...
      assert.strictEqual(cursor, "cursor-1", "prefetches with the server cursor");
    ...
    await render(
      <template><VideoHubLanding @model={{model}} @immersive={{true}} /></template>
    );
    ...
    assert
      ...
      .hasAttribute("data-active", "true", "keyboard navigation advances one card");
    assert.dom(".video-hub-player__iframe").exists({ count: 1 });
    assert.strictEqual(requestCount, 1, "does not fetch after pagination is exhausted");
  });
});

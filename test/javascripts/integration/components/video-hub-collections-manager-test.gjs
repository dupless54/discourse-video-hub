...
module("Integration | Component | VideoHubCollectionsManager", function (hooks) {
  ...
  test("renders visible and unavailable memberships without autoplay metrics", async function (assert) {
    ...
    assert
      ...
      .exists();
    assert
      .dom('.video-hub-collections__item-preview[href="/videos/91/visible-video"]')
  ...
  test("removes an unavailable membership without requiring hidden metadata", async function (assert) {
    ...
    await click(
      '[data-video-id="92"] .video-hub-collections__remove-video'
    );
